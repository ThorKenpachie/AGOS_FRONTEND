...
async function fetchModelJson(apiBaseUrl, path) {
  ...
  if (data.status !== 'success') throw new Error(data.message || 'Model API returned an error');
  return data;
}
...
export function useModelPrediction(modelKey = 'gru') {
  ...
  const fetchLatest = useCallback(async () => {
    try {
      const data = await fetchModelJson(apiBaseUrl, `/api/predict-flood/${modelKey}`);
      ...
      const normalized = {
        ...
        // Kept (not previously surfaced) so the Dashboard's transparency
        // panel can show real weather-data freshness and the backend's own
        // stated model reliability/assumptions instead of nothing at all.
        weather_cache:      data.weather_cache ?? null,
        meta:               data.meta ?? null,
      };
...
export function useFloodForecast14Day(modelKey = 'gru') {
  ...
  const fetchForecast14 = useCallback(async () => {
    try {
      const data = await fetchModelJson(apiBaseUrl, `/api/forecast-flood/${modelKey}`);
...
export function useModelComparison() {
  ...
  const fetchComparison = useCallback(async () => {
    try {
      const data = await fetchModelJson(apiBaseUrl, '/api/forecast-flood/compare');
